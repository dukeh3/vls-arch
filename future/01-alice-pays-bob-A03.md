...
Part of [Scenario 01 — Alice Pays Bob](01-alice-pays-bob.md). Follows [B02](01-alice-pays-bob-B02.md).
...
Same pattern as [A02](01-alice-pays-bob-A02.md): ValidateCommitmentTx2 reply is discarded, SignWithdrawal reply is needed.
...
Combines ValidateCommitmentTx2 + SignWithdrawal. The signer-proxy already has channel params from [A02](01-alice-pays-bob-A02.md)'s SetupChannel, so ValidateCommitmentTx2 values are mostly derivable. The truly new data is:
...
- Alice: CheckOutpoint + LockOutpoint + GetPerCommitmentPoint(1) → sends `channel_ready(ap1)` → [A04](01-alice-pays-bob-A04.md)
- Bob: CheckOutpoint + LockOutpoint + GetPerCommitmentPoint(1) → sends `channel_ready(bp1)` → [B03](01-alice-pays-bob-B03.md)
